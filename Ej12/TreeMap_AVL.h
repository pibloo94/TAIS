...
public:
   ...
   //------funcion que llama al metodo rangoClaves------
   void rango(int const& k1, int const& k2, std::vector<int>& vsol) {
	   rangoClaves(raiz, k1, k2, vsol);
   }
   
protected:
	//------metodo ejercicio------
	void rangoClaves(Link l, int const& k1, int const& k2, std::vector<int>& vsol) {
		if (l != nullptr) {
			if (l->cv.clave > k1) {
				rangoClaves(l->iz, k1, k2, vsol);
			}

			if (l->cv.clave <= k2 && l->cv.clave >= k1) {
				vsol.push_back(l->cv.clave);
			}

			if (l->cv.clave < k2) {
				rangoClaves(l->dr, k1, k2, vsol);
			}
		}
   }
